    ...
    if ModelOptimizationProcess== True:
        ...
        for Tindex in range(0, len(Battery5DischargeCapacity)):
            ...
            if TargetSOH <= Sp and RulActivation== True:
                ...
                plt.figure(figsize= (10, 4))
                plt.title("Charging Policy #3", fontsize= 20)
                plt.plot(Battery5Cycle[: len(SOHls)], SOHls, label= "Battery#8 (online prediction)")
                ...
                RulActivation= False
        plt.figure(figsize= (10, 4))
        plt.plot(Battery5Cycle[: Tsp], SOHls[: Tsp], label= "Actual Battery#8 (known)", color= "black")
        plt.plot(Battery5Cycle[Tsp: ], SOHls[Tsp: ], label= "Actual Battery#8 (unknown)", color= "darkblue")
