...
def FineTuneModelWeighsModule(FineTuneX, FineTuneY, PreTraindModel, FineTuneBatchSize, FineTuneEpoch, FinTuneLearningRate, Device):
    ...
    EpochLoss= []; EarlyStop= {"Iteration": 0, "LossValue": 10000, "MaxIteration": 3, "LastIteration": 0}
    ...
    plt.scatter(np.arange(0, len(EpochLoss)), EpochLoss, label= "Model fine-tuning loss curve", s= 20, color= "darkblue")
    plt.grid(True)
    plt.xlabel("Epoch", fontsize= 20)
    plt.ylabel("Fine-tuning loss", fontsize= 20)
    plt.xticks(fontsize= 20)
    plt.yticks(fontsize= 20)
    plt.legend(loc= "best", fontsize= 20)
    plt.show()
...
if __name__ == "__main__":
    ...
    if ModelOptimizationProcess== True:
        ...
        for Tindex in range(0, len(Battery4DischargeCapacity)):
            ...
            if TargetSOH <= Sp and RulActivation== True:
                ...
                plt.figure(figsize= (10, 4))
                plt.title("Charging Policy #3", fontsize= 20)
                plt.plot(Battery6Cycle[: len(SOHls)], SOHls, label= "Battery#8 (online prediction)")
                ...
                # OptParam: OptXStamp:   7// OptYStamp:   1// OptHiddenSize:  31// OptBatchSzie:  40// OptDropoutRate: 0.000000// OptEpoch: 762
                OptXTimeStamps= 7; OptYtimeStamps= 1; OptBatchSize= 40; Epoch= 100
                ...
                FineTunePretrainModel= FineTuneModelWeighsModule(FineTuneX, FineTuneY, PreTraindModel, OptBatchSize, Epoch, 0.00001, Device)
                ...
                RulActivation= False
        plt.figure(figsize= (10, 4))
        plt.plot(Battery6Cycle[: Tsp], SOHls[: Tsp], label= "Actual Battery#8 (known)", color= "black")
        plt.plot(Battery6Cycle[Tsp: ], SOHls[Tsp: ], label= "Actual Battery#8 (unknown)", color= "darkblue")
